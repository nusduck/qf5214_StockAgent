...
def main():
    ...
    parser.add_argument('--schedule', action='store_true', help='计划定时执行（默认立即执行一次后进入定时模式）')
    parser.add_argument('--hour', type=int, default=DEFAULT_HOUR, help=f'计划执行的小时（24小时制，默认{DEFAULT_HOUR}点）')
    parser.add_argument('--minute', type=int, default=DEFAULT_MINUTE, help=f'计划执行的分钟（默认{DEFAULT_MINUTE}分）')
    ...
    # 设置定时任务并开始循环
    # 如果只指定了--schedule，则只设置定时任务不立即执行
    # 如果没有指定任何参数，则默认立即执行一次，然后进入定时模式
    if args.schedule or not args.now:
        schedule_job(args.hour, args.minute, use_multithread)
        
        # 如果既没有指定立即执行，也没有指定计划执行的参数，则默认立即执行一次
        if not args.now and not args.schedule:
            logger.info(f"默认立即执行一次{thread_mode}增量数据获取，然后进入定时模式...")
            if use_multithread:
                run_incremental_multi()
            else:
                run_incremental_single()
        
        # 开始计划任务循环
        logger.info(f"开始计划任务循环，将在每天 {args.hour:02d}:{args.minute:02d} 自动执行，按Ctrl+C退出...")
        try:
            while True:
