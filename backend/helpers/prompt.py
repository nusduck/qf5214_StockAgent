...
fundamentals_prompt = """ 您是一位专注于中国股票市场的资深金融分析师，请基于最新财务数据为{stock_code}({stock_name})提供全面的基本面分析，并以专业研报格式输出。
...
## 2. 财务指标深度分析
### 以下是{stock_name}的主要财务指标分析，财务数据截至最近报告期{report_date}。（请将日期格式化为"年月日"格式，例如"2024年4月11日"。仅修改日期格式即可，此括号内的说明无需在报告中注明）。
### 盈利能力
   - 营业收入：{revenue}
   - 收入同比增长：{revenue_yoy}%
   - 净利润：{net_profit}
...
## 10. 目标价预测
 基于市盈率法(PE法)、市净率法(PB法)和ROE-PB模型法，得到综合估值法，预测未来三年(2025-2027)的目标价：
 1) 分析合理PE、PB倍数：
   - 请结合公司历史市盈率{pe}或滚动市盈率(TTM){pe_ttm}水平与同行业可比公司{pe}范围
   - 请结合公司历史市净率{pb}水平与同行业可比公司{pb}范围
   - 给出未来2025，2026，2027三个年度的合理市盈率{pe}、市净率{pb}估值区间，波动范围视情况不超过30%。
 2) 预测各项关键指标：
   - 基于历史财务数据和行业趋势，分析公司未来成长增速，波动范围视情况不超过30%
   - 预测2025-2027年的营业收入{revenue}，总市值：{total_mv}，EPS(每股收益){basic_eps}，BPS(每股净资产){net_asset_ps}，ROE(净资产收益率){roe}等，波动范围视情况不超过30%
 3) 计算目标价：
 市盈率PE法目标价 = 预测EPS{basic_eps} * 目标PE{pe}
 市净率PB法目标价 = 预测BPS{net_asset_ps} * 目标PB{pb}
 ROE-PB模型法目标价= 预测BPS{net_asset_ps} * ( b + a * 预测ROE{roe}%  ) 。其中以通信行业为例，a=10, b=0.2，a、b取值基于公司所在行业的回归系数，可自定。
 综合估值法目标价 = 市盈率PE法目标价 * 0.4 + 市净率PB法目标价* 0.3 + ROE-PB模型法目标价 * 0.3
 根据以上公式分别计算2025,2026,2027年目标价 ，并给出合理波动区间，波动范围视情况不超过30%

 请按以下格式输出结果：
 - 先用1-2句话简要说明估值逻辑和主要考虑因素。基于市盈率法(PE法)、市净率法(PB法)和ROE-PB模型法，按照权重计算后进行综合估值，对{stock_name}未来的目标价做出以下预测：
 - 2025年目标价：预估目标价约XX元，可能波动区间XX元~XX元，主要驱动因素包括XXX
 - 2026年目标价：预期在公司业绩逐步兑现与估值修复推动下，目标价有望达到XX元左右，关键驱动因素包括XXX
 - 2027年目标价：中长期看，在全球新能源发展和公司战略实施的推动下，目标价预测约XX元，长期价值主要依赖于XX
 - 关键催化剂时间表：关注下一财季业绩预告、国家政策调整及XX数据，这些都可能在未来3-6个月内对股价形成重要催化作用。
 - 请注意，以上数据仅供参考，实际投资需谨慎决策。

 注意：保持预测的合理性，不要超出历史最高价的30%，除非有极强的业绩支撑证据。目标价应基于实际财务数据和市场表现。PE倍数合理递减，考虑到市场波动和公司成长性。请确保所有数据准确无误，并在必要时注明数据来源。
