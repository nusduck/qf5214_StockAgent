...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(current_dir, "multi_thread_incremental.log")),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

def run_download_task(func, name, max_symbols=None):
    """每个任务创建自己的数据库连接"""
    connection = None
    try:
        # 从连接池获取一个连接
...
def main():
    ...
    logger.info(f"开始多线程增量数据获取，最大线程数: {max_workers}，最大股票数: {max_symbols if max_symbols else '不限'}")
